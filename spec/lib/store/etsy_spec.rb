...
describe MadCart::Store::Etsy do # rubocop:disable Metrics/BlockLength
  before(:each) { clear_config }

  describe 'retrieving products' do # rubocop:disable Metrics/BlockLength
    context "the store doesn't exist" do
      let(:invalid_store_name) { 'MadeUpStore' }
      let(:store) do
        MadCart::Store::Etsy.new(store_name: invalid_store_name,
                                 api_key: '4j3amz573gly866229iixzri')
      end

      it 'raises an exception' do
        VCR.use_cassette('etsy_store_does_not_exist') do
          expect { store.products }
            .to raise_exception(MadCart::Store::InvalidStore)
        end
      end
    end

    context 'the store does exist' do # rubocop:disable Metrics/BlockLength
      before(:each) do
      ...
      it 'returns products' do
        VCR.use_cassette('etsy_store_listings') do
          ...
          expect(products.size).to eq(25) # the etsy product limit
        ...
        it 'returns the page requested' do
          VCR.use_cassette('etsy_store_listings') do
            ...
            expect(api.connection)
              .to receive(:listings)
              .with(:active, page: 2)
              .and_return([]) # Trusting the Etsy gem, not testing that it works
            api.products(page: 2)
          end
        end
      end
